...
from config import (SPLITS_DIR, BEST_MODEL_PATH, LOG_DIR,
                    BATCH_SIZE, SEQ_LEN, OVERLAP, CLASSES)
...
def predict(model, loader, device, num_classes=2, threshold=0.5):
    ...
    for seqs, lbs in loader:
        ...
        if num_classes == 2:
            ...
            # Class 1 (Anomaly) prediction based on threshold
...
def plot_cm(y_true, y_pred, names, path):
    ...
    sns.heatmap(cm.astype(float)/cm.sum(1,keepdims=True), annot=cm, fmt="d",
                cmap="Blues", xticklabels=names, yticklabels=names, ax=ax)
    ax.set(xlabel="Predicted", ylabel="True",
           title="UCF-Crime LSTM — Confusion Matrix")
...
def evaluate(args):
    ...
    # ── Use custom test CSV if provided ──
    test_csv = args.test_csv or os.path.join(SPLITS_DIR, "test.csv")

    ds = UCFCrimeDataset(test_csv,
                         SEQ_LEN, OVERLAP, augment=False)
    ...
    if len(ds) == 0:
        raise RuntimeError(
            f"No test windows found. Check that paths in {test_csv} "
            f"point to existing .npy files."
        )
    ...
    summary = (f"Test Accuracy : {acc*100:.2f}%\n"
               f"Macro F1      : {f1_mac*100:.2f}%\n"
               f"Weighted F1   : {f1_wt*100:.2f}%\n\n{report}")
    print("\n" + summary)

    os.makedirs(LOG_DIR, exist_ok=True)
    with open(os.path.join(LOG_DIR,"test_metrics.txt"),"w") as f: f.write(summary)
...
if __name__ == "__main__":
    ...
    p.add_argument("--test_csv",   default=r"D:\Threat Detection\splits\test_binary.csv",
                   help="Override default test CSV path")
    p.add_argument("--num_classes", type=int, default=2,
                   help="Override NUM_CLASSES (e.g. 2 for binary)")
    p.add_argument("--threshold", type=float, default=0.5,
                   help="Probability threshold for predicting Anomaly class in binary mode (default: 0.5)")
