...
from config import YOLO_MODEL_PATH, LSTM_CHECKPOINT_PATH, CONF_BASE, CONF_GENERIC_WEAPON, CONF_ARMED_PERSON, MIN_BOX_AREA_RATIO, WEAPON_CLASSES, ARMED_PERSON_CLASSES, PERSON_CLASSES, CLASSES, NUM_CLASSES, IDX_TO_CLASS, NORMAL_IDX, CLASS_SEVERITY, CNN_BACKBONE, CNN_FEATURE_DIM, IMG_SIZE, SEQ_LEN, OVERLAP, LSTM_ALERT_THRESHOLD, CAMERA_LOCATION
...
def _get_transform():
    ...
    if _IMG_TRANSFORM is None:
        _IMG_TRANSFORM = transforms.Compose([transforms.Resize(IMG_SIZE), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
...
def _build_cnn(backbone: str, device):
    ...
    if backbone == 'resnet50':
        ...
        m.classifier = nn.Sequential(nn.Dropout(p=0.2, inplace=True), nn.Linear(1280, CNN_FEATURE_DIM))
    else:
...
def is_valid_detection(label, conf, x1, y1, x2, y2, frame_h, frame_w) -> bool:
    ...
    if box_area_ratio < MIN_BOX_AREA_RATIO:
        return False
    if label in WEAPON_CLASSES and box_area_ratio > 0.2:
        return False
...
def run_yolo(frame, h, w):
    results = yolo_model.predict(frame, conf=CONF_BASE, iou=0.5, verbose=False, device='cpu')[0]
    persons = weapons = 0
    dets = []
    for box in results.boxes:
        ...
        cv2.putText(frame, lbl_text, (x1 + 2, max(y1 - 4, th)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)
...
class FeedState:
    ...
    def __init__(self, feed_id: str, url: str, is_video: bool=False):
        ...
        self.stats = {'feed_id': feed_id, 'url': url, 'is_video': is_video, 'person_count': 0, 'weapon_count': 0, 'total_alerts': 0, 'fps': 0, 'status': 'idle', 'last_updated': None, 'lstm_label': '—', 'lstm_confidence': 0.0, 'lstm_severity': 'CLEAR', 'lstm_loaded': lstm_loaded}
...
def detection_loop(feed_id: str):
    ...
    while not feed.stop_flag:
        ...
        feed.stats.update({'person_count': persons, 'weapon_count': weapons, 'last_updated': datetime.now().strftime('%H:%M:%S'), 'lstm_label': feed.lstm_engine.label, 'lstm_confidence': round(feed.lstm_engine.confidence, 3), 'lstm_severity': feed.lstm_engine.severity})
        if weapons > 0:
        ...
        if consecutive_weapon_frames >= 2:
            ...
            if consecutive_weapon_frames == 2:
                ...
                alert_history.appendleft({'id': total_alerts, 'feed_id': feed_id, 'time': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'type': 'WEAPON DETECTED', 'weapon_count': weapons, 'person_count': persons, 'detections': dets, 'location': CAMERA_LOCATION['name'], 'severity': 'HIGH', 'behavior': feed.lstm_engine.label, 'behavior_conf': round(feed.lstm_engine.confidence, 3), 'behavior_severity': feed.lstm_engine.severity})
        elif lstm_loaded and feed.lstm_engine.label not in ('—', 'Normal') and (feed.lstm_engine.confidence >= LSTM_ALERT_THRESHOLD):
            feed.stats['status'] = 'alert'
            if frame_count % 30 == 0:
                total_alerts += 1
                feed.stats['total_alerts'] += 1
                alert_history.appendleft({'id': total_alerts, 'feed_id': feed_id, 'time': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'type': 'ANOMALY DETECTED', 'weapon_count': 0, 'person_count': persons, 'detections': [], 'location': CAMERA_LOCATION['name'], 'severity': 'CRITICAL', 'behavior': feed.lstm_engine.label, 'behavior_conf': round(feed.lstm_engine.confidence, 3), 'behavior_severity': feed.lstm_engine.severity})
        else:
            feed.stats['status'] = 'monitoring'
        fps_counter += 1
        ...
        cv2.putText(frame, id_txt, (8, 19), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1, cv2.LINE_AA)
        if lstm_loaded:
            if feed.lstm_engine.label != '—':
                sev_col = (0, 200, 0) if feed.lstm_engine.severity == 'CLEAR' else (0, 180, 255) if feed.lstm_engine.severity in ('LOW', 'MEDIUM') else (0, 0, 220)
        ...
        cv2.putText(frame, lstm_txt, (w - tw - 8, 19), cv2.FONT_HERSHEY_SIMPLEX, 0.55, sev_col, 1, cv2.LINE_AA)
...
def api_restart_feed(feed_id):
    ...
    feed.frame_b64 = None
    feed.lstm_engine.reset()
    feed.stats.update({'person_count': 0, 'weapon_count': 0, 'fps': 0, 'status': 'idle', 'last_updated': None, 'lstm_label': '—', 'lstm_confidence': 0.0, 'lstm_severity': 'CLEAR'})
    feed.thread = threading.Thread(target=detection_loop, args=(feed_id,), daemon=True)
    feed.thread.start()
...
def api_feed_stats(feed_id):
    ...
    if feed.lstm_engine.probs:
        info['probs'] = {CLASSES[i]: round(float(p), 3) for i, p in enumerate(feed.lstm_engine.probs)}
    return jsonify(info)
...
def api_global_stats():
    ...
    return jsonify({'person_count': total_persons, 'weapon_count': total_weapons, 'total_alerts': total_alerts, 'active_feeds': active_feeds, 'status': overall_status, 'last_updated': datetime.now().strftime('%H:%M:%S'), 'lstm_loaded': lstm_loaded})
...
def api_upload():
    ...
    return jsonify({'job_id': 'simulated', 'status': 'playing_in_live_feed', 'feed_id': result['feed_id']})
